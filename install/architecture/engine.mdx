...
- **Extract Addon Metadata**: Extracts metadata when installing new addons.
...
The engine takes the flow JSON with an engine token scoped to this project and implements the API provided for the addon framework, such as:
- Storage Service: A simple key/value persistent store for the addon framework.
