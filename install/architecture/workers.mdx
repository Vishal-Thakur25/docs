...
The difference between the two modes is in the execution of code addons. For V8 Sandboxing, we use [isolated-vm](https://www.npmjs.com/package/isolated-vm), which relies on V8 isolation to isolate code addons.

These are the steps that are used to execute the flow:

<Steps>
  <Step title="Prepare Code Addons">
...
#### Security:
In a self-hosted environment, all addon installations are done by the **platform admin**. It is assumed that the addons are secure, as they have full access to the machine.

Code addons provided by the end user are isolated using V8, which restricts the user to browser JavaScript instead of Node.js with npm.
