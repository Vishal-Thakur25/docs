---
title: "AWS (Pulumi)"
description: "Get NRAI up & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy NRAI in a few clicks. 
The template will deploy the latest NRAI image that's available on [Docker Hub](https://hub.docker.com/r/NRAI/NRAI).

[![Deploy with Pulumi](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/NRAI/NRAI/tree/main/deploy/pulumi)

## Deploy from a local environment

Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy NRAI direct from Docker Hub using the template below in just few commands:

```bash
$ mkdir deploy-NRAI && cd deploy-NRAI 
$ pulumi new https://github.com/NRAI/NRAI/tree/main/deploy/pulumi
$ pulumi up
```
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your NRAI deployment.

For a full list of all the currently available configuration options, take a look at the [NRAI Pulumi template file on GitHub](https://github.com/NRAI/NRAI/tree/main/deploy/pulumi/Pulumi.yaml).

## Setting up Pulumi for the first time

If you're new to Pulumi then read on to get your local dev environment setup to be able to deploy NRAI.
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our NRAI deployment:
...
```
6. Next we're going to use the NRAI Pulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
<Tip>
    Note: When choosing a stack name, use something descriptive like `NRAI-dev`, `ap-prod` etc. 
...
```json
    _: {
        nraiUrl: "http://<alb name & id>.us-east-1.elb.amazonaws.com"
        NRAIEnv: [
         . . . .
        ]
       }
...
The config value of interest here is the `nraiUrl` as that is the URL for our NRAI deployment. 
...
Congratulations! You have successfully deployed NRAI to AWS.

## Deploy a locally built NRAI Docker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the NRAI repo locally:

```bash
git clone https://github.com/NRAI/NRAI
```
...
```
3. This folder already has two Pulumi stack configuration files reday to go: `Pulumi.NRAI-dev.yaml` and `Pulumi.NRAI-prod.yaml`. 
...
Lets continue by creating a development stack that uses the existing `Pulumi.NRAI-dev.yaml` file & kick off the deploy.

```bash
pulumi stack init NRAI-dev && pulumi up
```

<Tip>
    Note: Using `NRAI-dev` or `NRAI-prod` for the `pulumi stack init` command is required here as the stack name needs to match the existing stack file name in the folder. 
</Tip>
...
Congratulations! You have successfully deployed NRAI into AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official NRAI Docker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.   
