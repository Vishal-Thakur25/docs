...
<Steps>
    ...
    <Step title="Call Connect Method">
        After initializing the SDK, you will have access to a property called `NRAI` inside your `window` object. Call its `connect` method to open a new connection dialog as follows.

        ```html
        <script> 
        NRAI.connect({addonName:'@NRAI/addon-google-sheets'});
        </script>
        ```
        ...
        **Connect Parameters:**
        ...
        | -------------- | -------- | ------ | ------------------------------------------- |
        | addonName      | ✅      | string | The name of the addon you want to create a connection for.
        ...
        **Connect Result**
        ...
        <Tip>
        You can use the `connections` addon in the builder to retrieve the created connection using its name.
        ![Connections in Builder](/resources/screenshots/connections-addon.png)
        ![Connections in Builder](/resources/screenshots/connections-addon-usage.png)
        </Tip>
    </Step>
</Steps>
