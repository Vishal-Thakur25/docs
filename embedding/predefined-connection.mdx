...
Use predefined connections to allow users to access your addon in the embedded app without re-entering authentication credentials.
...
- (Optional) Hide the connections dropdown in the addon settings.

### Prerequisites

- [Run the Enterprise Edition](/handbook/engineering/playbooks/run-ee)
- [Create your addon](/developers/building-addons/overview). Later we will customize the addon logic to use predefined connections.
...
const instanceUrl = 'https://cloud.NRAI.com';

// The name of the user / organization in your SAAS
const externalProjectId = 'org_1234';
const addonName = '@NRAI/addon-gelato';
// This will depend on what your addon auth type is, can be one of this ['PLATFORM_OAUTH2','SECRET_TEXT','BASIC_AUTH','CUSTOM_AUTH']
const addonAuthType = "CUSTOM_AUTH"
const connectionProps = {
    // Fill in the props required by your addon's auth
}
...
<Steps>
    <Step title="Modify Trigger / Action Definition">
       Wherever you call `createTrigger` or `createAction` set `requireAuth` to `false`, this will hide the connections dropdown in the addon settings in the builder,
...
const fetchConnection = async (
...
): Promise<AddonPropValueSchema<typeof auth>> => {
	...
	return connection as AddonPropValueSchema<typeof auth>;
};
