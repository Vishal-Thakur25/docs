...
window.addEventListener("popstate", () => {
  const route = NRAI.extractNRAIRouteFromUrl({ vendorUrl: window.location.href });
  NRAI.navigate({ route });
});
```
...
If you use `NRAI.navigate({ route: '/flows' })` this will tell the embedded sdk where to navigate to.
