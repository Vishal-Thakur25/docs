...
To avoid making the dev environment slow, not all addons are functional during development at first. By default, only these addons are functional at first, as specified in `AP_DEV_PIECES`.

https://github.com/NRAI/NRAI/blob/main/packages/server/api/.env#L4

To override the default list available at first, define an `AP_DEV_PIECES` environment variable with a comma-separated list of addons to make available. For example, to make `google-sheets` and `cal-com` available, you can use:
