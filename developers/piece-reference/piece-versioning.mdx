---
title: 'Addon Versioning'
icon: 'code-compare'
description: 'Learn how to version your addons'
---

Addons are npm packages and follows **semantic versioning**.

## Semantic Versioning

The version number consists of three numbers: `MAJOR.MINOR.PATCH`, where:

- **MAJOR** It should be incremented when there are breaking changes to the addon.
...
## Engine
The engine will use the most up-to-date compatible version for a given addon version during the **DRAFT** flow versions. Once the flow is published, all addons will be locked to a specific version.

**Case 1: Addon Version is Less Than 1.0**:
The engine will select the latest **patch** version that shares the same **minor** version number.

**Case 2: Addon Version Reaches Version 1.0**:
