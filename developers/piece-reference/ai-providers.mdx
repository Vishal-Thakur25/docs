...
icon: 'brain'
description: 'The AI Toolkit to build AI addons tailored for specific use cases that work with many AI providers using the AI SDK'
---
...
NRAI integrates with the [AI SDK](https://ai-sdk.dev/) to provide a unified interface for calling LLMs across multiple AI providers. Here's an example on how to use the AI SDK's `generateText` function to call an LLM in your actions.

```typescript
import { SUPPORTED_AI_PROVIDERS, createAIModel, aiProps } from '@NRAI/common-ai';
import { createAction, Property } from '@NRAI/addons-framework';
...
```typescript
import { aiProps } from '@NRAI/ai-common';
...
To add advanced options for your new provider, update the `advancedOptions` property in `packages/addons/community/common/src/lib/ai/index.ts`:

```typescript
// In packages/addons/community/common/src/lib/ai/index.ts
...
To add support for a new AI provider, you need to update several files in the NRAI codebase. Here's a complete guide:
...
2. Create a new API key for NRAI integration.
3. Add your credit card and upgrade to paid plan to avoid rate limits.
`,                                                        // Instructions for users
    logoUrl: 'https://cdn.NRAI.com/addons/openai.png', // OpenAI logo
    auth: {
...
Once these changes are made, your new AI provider will be available in the `aiProps` dropdowns and can be used with `generateText` and other [AI SDK functions](https://ai-sdk.dev/docs/introduction) throughout NRAI.
