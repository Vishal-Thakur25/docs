...
The NRAI repository is structured as a monorepo, employing Nx as its build tool.
...
- Import the library into your addon.

Guidelines:
- Make sure you are using well-maintained libraries.
- Ensure that the library size is not too large to avoid bloating the bundle size; this will make the addon load faster in the sandbox.