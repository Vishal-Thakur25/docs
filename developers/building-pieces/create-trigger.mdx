...
This tutorial will guide you through the process of creating trigger for a Gelato addon that fetches new icecream flavor.
...
1. `Addon Folder Name`: This is the name associated with the folder where the trigger resides. It helps organize and categorize triggers within the addon.
...
4. `Trigger Technique`: Specifies the trigger type - either [polling](../addon-reference/triggers/polling-trigger) or [webhook](../addon-reference/triggers/webhook-trigger).
...
? Enter the addon folder name : gelato
...
This will create a new TypeScript file at `packages/addons/community/gelato/src/lib/triggers` named `new-flavor-created.ts`.
...
import {
  ...
  httpClient,
  pollingHelper,
} from '@NRAI/addons-common';
import {
  AddonPropValueSchema,
  TriggerStrategy,
  createTrigger,
} from '@NRAI/addons-framework';
import dayjs from 'dayjs';

const polling: Polling<
  AddonPropValueSchema<typeof gelatoAuth>,
  Record<string, never>
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const request: HttpRequest = {
      ...
      url: 'https://cloud.NRAI.com/api/v1/webhooks/aHlEaNLc6vcF1nY2XJ2ed/sync',
      headers: {
        authorization: auth,
      },
    };
...
To make the trigger readable by NRAI, add it to the array of triggers in the addon definition.

```typescript
import { createAddon } from '@NRAI/addons-framework';
...
For more details, check out the [Addon Development](../development-setup/getting-started) section.

Once you edit the environment variable, restart the backend. The addon will be rebuilt. After this process, you'll need to **refresh** the frontend to see the changes.

To test the trigger, use the load sample data from flow builder in NRAI. It should function as shown in the screenshot.
