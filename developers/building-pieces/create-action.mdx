...
1. `Addon Folder Name`: This is the name associated with the folder where the action resides. It helps organize and categorize actions within the addon.
...
This will create a new TypeScript file named `get-icecream-flavor.ts` in the `packages/addons/community/gelato/src/lib/actions` directory.
...
import { httpClient, HttpMethod } from '@NRAI/addons-common';
import { gelatoAuth } from '../..';

export const getIcecreamFlavor = createAction({
  name: 'get_icecream_flavor', // Must be a unique across the addon, this shouldn't be changed.
  auth: gelatoAuth,
  ...
  async run(context) {
    const res = await httpClient.sendRequest<string[]>({
      ...
      url: 'https://cloud.NRAI.com/api/v1/webhooks/RGjv57ex3RAHOgs0YK6Ja/sync',
      headers: {
...
To make the action readable by NRAI, add it to the array of actions in the addon definition.
...
To test the action, use the flow builder in NRAI. It should function as shown in the screenshot.
