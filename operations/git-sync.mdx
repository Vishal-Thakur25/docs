...
- Two Projects in NRAI: one for Development and one for Production
...
<Tip>
Don't forget to replace `INSTANCE_URL` and `PROJECT_ID`, and add `NRAI_API_KEY` to the secrets.
</Tip>
      ...
      - name: deploy
        ...
        run: |
          curl --request POST \
            ...
            --header 'Authorization: Bearer ${{ secrets.NRAI_API_KEY }}' \
